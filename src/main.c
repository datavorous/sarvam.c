...
int main(int argc, char *argv[]) {
  ...
  Config cfg;
  float *data = load_weights(argv[1], &cfg);
  ...
  int max_token_len;
  fread(&max_token_len, sizeof(int), 1, tf);
  ...
  int token = final_tokens[0], prev = 1;
  for (int pos = 0; pos < steps; pos++) {
    float *logits =
        forward(&cfg, &w, &s, pos < n ? final_tokens[pos] : token, pos);
